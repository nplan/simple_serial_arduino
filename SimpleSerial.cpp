...
SimpleSerial::SimpleSerial(Stream *serial) {
  _serial = serial;
}
...
void SimpleSerial::send(uint8_t id, uint8_t bytesLen, uint8_t bytes[]) {
  ...
  if(_sendQueueLen >= QUEUE_LEN)
    return;
  // Frame packet
  uint8_t frame[MAX_PCKT_LEN];
  uint8_t frameLen;
  this->frame(id, bytesLen, bytes, frameLen, frame);
  // Place packet in send waitlsit
  uint8_t queueIndex = _sendQueueLen;
  for(uint8_t i=0; i<frameLen; i++) {
    // Copy data to array
    _sendQueue[queueIndex][i] = frame[i];
  }
  _sendQueueLengths[queueIndex] = frameLen;
  _sendQueueLen++;
}
...
void SimpleSerial::frame(uint8_t id, uint8_t payloadLen, uint8_t payload[],
  ...
  // Insert ESC flags
  uint8_t i = 0; // bytesIn index
  uint8_t j = 0; // bytesOut index
  packet[0] = START;
  packet[1] = 0; //length
  packet[2] = id;
  j = 3;
...
/*
 * Continuously running loop. If available, sends data byte by byte.
 * Sends multiple bytes per loop, specified by SEND_BYTES.
 */
void SimpleSerial::sendLoop(uint32_t time) {
  static int i = 0;
  static int j = 0;

  for (uint8_t k=0; k<SEND_BYTES; k++)
  {
    if(_sendQueueLen == 0)
    ...
    if(i<_sendQueueLen) {
      // Loop through bytes in a packet
      if(j<_sendQueueLengths[i]) {
        _serial->write(_sendQueue[i][j]);
        j++;
      }
      else {
        // End of packet, go to next.
        j = 0;
        i++;
      }
    }
...
/*
 * Returns and removes the oldest packet in the read list.
 */
...
/*
 * Continuously running loop. Waits for bytes as they arive and
 * decodes the packet.
 */
void SimpleSerial::readLoop(uint32_t time) {
  ...
  static unsigned long startTime = time;
